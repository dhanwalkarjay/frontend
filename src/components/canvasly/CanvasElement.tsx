...
export function CanvasElement({ element, canvasBoundsRef }: CanvasElementProps) {
      ...
      case 'image':
        return (
          <NextImage // Use aliased import
            ...
            className="object-contain pointer-events-none w-full h-full rounded-sm" // rounded-sm for image itself
            data-ai-hint={element['data-ai-hint'] as string || "placeholder image"}
            draggable={false}
            priority={true} // For initial images, might adjust later
          />
        );
      case 'text':
        if (isEditingText) {
          return (
            <Textarea
              ...
              onClick={(e) => e.stopPropagation()} // Prevent click from bubbling to div
        ...
        return (
          <div 
            className="w-full h-full flex items-center justify-center p-1 break-words overflow-hidden whitespace-pre-wrap" // whitespace-pre-wrap to respect newlines
            style={{ fontSize: `${element.fontSize || 16}px`, color: element.textColor || 'hsl(var(--foreground))' }}
  ...
  return (
    <div
      ref={elementRef}
      className={cn(
        "absolute cursor-grab select-none group", // Add group for hover effects on children
        "flex items-center justify-center",
        "shadow-md hover:shadow-lg transition-shadow duration-200",
        "bg-card/70 backdrop-blur-sm rounded-md border border-transparent", // More subtle border initially
      ...
      onMouseDown={(e) => { 
        if (!isEditingText) { 
          handleMouseDown(e); // This calls selectElement via onClick
        }
        // Always ensure selection happens on mousedown for immediate feedback and z-index update
        if (selectedElementId !== element.id) {
          selectElement(element.id);
        }
      }}
      // onClick handles cases where mousedown might be stopped by other logic or for accessibility
      onClick={handleClick} 
